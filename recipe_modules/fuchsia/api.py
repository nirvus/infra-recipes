...
class FuchsiaApi(recipe_api.RecipeApi):
  ...
  def _tar_fuchsia_packages(self, build_results):
    ...
    Returns:
      A Path to a tarball containing Fuchsia packages.
    """
    # Begin creating Fuchsia packages tar package.
    package = self.m.tar.create(
        self.m.path['cleanup'].join('packages.tar.gz'), 'gzip')
    ...
    package.add(amber_repo_dir.join('targets'), amber_repo_dir)
    package.add(amber_repo_dir.join('blobs'), amber_repo_dir)
    ...
    package.add(
        host_build_dir.join('amber-publish'), build_results.fuchsia_build_dir)
    package.add(
        host_build_dir.join('pm'), build_results.fuchsia_build_dir)

    # Tar the files.
    package.tar('tar fuchsia packages')
